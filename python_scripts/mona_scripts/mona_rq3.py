"""
RQ3 ENTROPY ANALYSIS - MONA SCRIPT
...
Approach:
    1. Load RQ2 municipality predictability results (rq2_kommun_predictability.csv)
    2. Load raw data and compute Y distribution per municipality
    3. Compute Shannon entropy, majority class proportion, Herfindahl index
    4. Merge with predictability metrics
    5. Run descriptive stats, correlation, and regression analyses
    6. Save results to CSV

Expected Runtime: ~a few minutes on MONA
...
# %% MONA PATHS - UPDATE THESE IF NEEDED
...
def regression_analysis(df, logger):
    ...
    Now includes p-values and 95% confidence intervals.
    """
    logger.info("=" * 80)
    logger.info("REGRESSION ANALYSIS (with p-values + 95% CI)")
    logger.info("=" * 80)
    logger.info("")
    ...
    # ---------------------------
    # Model 1: Accuracy ~ Entropy
    # ---------------------------
    X1 = sm.add_constant(df['entropy'])
    model1 = sm.OLS(df['balanced_accuracy'], X1).fit()
    ...
    rows.append({
        ...
        'coef_entropy': model1.params['entropy'],
        'coef_log_n_test': None,
        'p_entropy': model1.pvalues['entropy'],
        'ci_entropy_low': model1.conf_int().loc['entropy', 0],
        'ci_entropy_high': model1.conf_int().loc['entropy', 1],
        'r2': model1.rsquared,
        'n_obs': n_obs
    })

    # ---------------------------
    # If n_test exists: add models 2 and 3
    # ---------------------------
    if 'n_test' in df.columns:

        # Model 2: Accuracy ~ log(n_test)
        X2 = sm.add_constant(np.log(df['n_test']))
        model2 = sm.OLS(df['balanced_accuracy'], X2).fit()
        ...
        rows.append({
            ...
            'coef_entropy': None,
            'coef_log_n_test': model2.params[np.log(df['n_test']).name],
            'p_log_n_test': model2.pvalues[np.log(df['n_test']).name],
            'ci_log_low': model2.conf_int().iloc[1, 0],
            'ci_log_high': model2.conf_int().iloc[1, 1],
            'r2': model2.rsquared,
            'n_obs': n_obs
        })

        # Model 3: Accuracy ~ Entropy + log(n_test)
        X3 = sm.add_constant(
            np.column_stack([df['entropy'], np.log(df['n_test'])])
        )
        model3 = sm.OLS(df['balanced_accuracy'], X3).fit()

        logger.info("Model 3: balanced_accuracy ~ entropy + log(n_test)")
        logger.info(model3.summary())
        ...
        rows.append({
            ...
            'coef_entropy': model3.params[1],
            'coef_log_n_test': model3.params[2],
            'p_entropy': model3.pvalues[1],
            'p_log_n_test': model3.pvalues[2],
            'ci_entropy_low': model3.conf_int().iloc[1, 0],
            'ci_entropy_high': model3.conf_int().iloc[1, 1],
            'ci_log_low': model3.conf_int().iloc[2, 0],
            'ci_log_high': model3.conf_int().iloc[2, 1],
            'r2': model3.rsquared,
            'n_obs': n_obs
        })
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("RQ3: ENTROPY ANALYSIS (MONA VERSION)")
