"""
RQ2 GEOGRAPHIC ANALYSIS - DEVELOPMENT SCRIPT
...
FOR PRODUCTION: Use mona_rq2.py in MONA

Expected Runtime: 
- Local (test data): ~0-1 minute
- MONA (full data): ~10-15 minutes

Main Outputs: 
-   outputs_synthetic\tables\rq2_kommun_predictability.csv  # Metrics per municipality
-   outputs_synthetic\tables\rq2_summary_statistics.csv      # Summary statistics across municipalities
"""
...
def load_best_model(config, logger):
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            ...
            "Please run run_rq1_full_training.py first!"
        )
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("RQ2: GEOGRAPHIC ANALYSIS (DEVELOPMENT VERSION)")
    ...
    logger.info("")
    logger.info("Expected runtime: 1-2 minutes (test data)")
    logger.info("=" * 80)
    logger.info("")
    ...
    try:
        ...
        print_top_bottom_kommuner(kommun_df, logger, n=min(10, len(kommun_df)))
        ...
        logger.info("  1. Create choropleth maps with local visualization scripts")
        logger.info("  2. Analyze patterns: urban vs rural, homogeneous vs diverse")
        logger.info("=" * 80)
