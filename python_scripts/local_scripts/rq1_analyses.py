"""
RQ1 FULL TRAINING - DEVELOPMENT SCRIPT
...
FOR LOCAL TESTING: Uses test data (40k observations)
FOR PRODUCTION: Use mona_rq1.py in MONA

Expected Runtime: 
- Local (test data): ~2-5 minutes
- MONA (full data): ~12-14 hours

Main Outputs: 
-   outputs_synthetic\tables\rq1_all_experiments.csv  # Comparing across different models (stratified vs balanced, across different train sizes)
-   outputs_synthetic\tables\rq1_pareto_frontier.csv    # To pick the best model
-   outputs_synthetic\models\rq1_best_model.pkl     # The best model to be used 
"""
...
from mona_scripts.data_loader import load_data, preprocess_data, split_data
from mona_scripts.sampler import sample_data
from mona_scripts.trainer import train_model, evaluate_model
...
DATA_PATH = "data/synthetic_data.dta"  # Still needed for config setup
...
config.COMPARISON_SIZES = [0.1, 0.2, 0.3, 0.4]  # In thousands
config.COMPARISON_METHODS = ['stratified', 'balanced']

# Scaling zone: 1M-4M (stratified only)
config.SCALING_SIZES = [1, 2, 3, 4]  # In thousands

# Seeds: 3 seeds for ≤200k, 1 seed for >200k
all_sizes = config.COMPARISON_SIZES + config.SCALING_SIZES
config.SEED_CONFIG = {
    size: [42, 123, 456] if size <= 5 else [42]
